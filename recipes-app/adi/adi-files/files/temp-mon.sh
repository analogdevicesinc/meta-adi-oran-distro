...
#fetch env and judge the platform id
result=$(cat /etc/environment)
if [[ $result == *"kerberos"* ]]; then
    PLATFORM=kerberos
else
    wall "Board error: The temp monitoring isn't supported for ${result}!"
    exit 1
fi

#I2C bus number for temp sensor is 4
TEMP_BUS=4
NUM_BUS=8
...
do
    ...
    if [[ "$overtemp_sig" -gt "0" && "$i" == "0" ]]; then
        wall "Board error: Rework the board is needed for the temp monitoring!"
        exit 1
    fi
